...
from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger            # Phân trang
from cart.cart import Cart
from django.http import JsonResponse            # Web Service
...
def trang_chu(request):
    ...
    # Dụng cụ nhà bếp => id=2
    subcategory_id_dcnb = SubCategory.objects.filter(category=2).values('id')
    list_subcategory_id_dcnb = [sub_id['id'] for sub_id in subcategory_id_dcnb]
    products_dcnb = Product.objects.filter(subcategory__in=list_subcategory_id_dcnb).order_by('public_day')[:20]
...
# def master(request):
#     return render(request, 'store/_Master.html')
...
def danh_muc(request, pk):
    ...
    if request.GET.get('gia'):
        ...
        if pk == 0:
            ...
            # title_subcategory = f'Tìm thấy {len(products)} sản phẩm với giá 3000000 trở lên'
            if den_gia != '':
                products = Product.objects.filter(price__gte=tu_gia, price__lt=den_gia).order_by('-public_day')
                # title_subcategory = f'Tìm thấy {len(products)} sản phẩm với giá {range_gia}'
                if tu_khoa != '':
    ...
    return render(request, 'store/product-list.html', {
        'products': products,
        'products_pager': products_pager,               # Phân trang
        'subcategories': subcategories, 
        'title_subcategory': title_subcategory,
        'brands': brands,
        'cart': cart,
        'range_gia': range_gia,                      # Lọc theo giá
        'keyword': tu_khoa
    })
...
# Cách 2: WebService từ thư viện django rest_framework
