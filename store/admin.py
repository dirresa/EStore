...
class ProductAdmin(admin.ModelAdmin):
    exclude = ('public_day', 'viewed')                                                    # Ẩn các field (không cho nhập)
    # list_display = ('name', 'price', 'public_day', 'viewed', 'subcategory',)            # Hiển thị các field trên danh sách
    list_display = ('e_name', 'e_price', 'e_public_day', 'e_viewed', 
                    'e_subcategory', 'e_category', 'e_image') 
    list_filter = ('public_day',)                                                         # Hiển thị danh sách theo filter
    search_fields = ('name__contains',)                                                   # Tìm kiếm
